...
unsigned short indirect_indexed_addr(magic6502_ctx* ctx) {
  return ctx->address_accessed = (fetch_short(ctx, *ctx->ma(ctx, ctx->pc + 1)) + ctx->y);
}

unsigned char zero_page_addr(magic6502_ctx* ctx) {
  return ctx->address_accessed = *ctx->ma(ctx, ctx->pc + 1);
}

unsigned char zero_page_x_addr(magic6502_ctx* ctx) {
  return ctx->address_accessed = (*ctx->ma(ctx, ctx->pc + 1) + ctx->x);
}

unsigned char zero_page_y_addr(magic6502_ctx* ctx) {
  return ctx->address_accessed = (*ctx->ma(ctx, ctx->pc + 1) + ctx->y);
}
...
unsigned short absolute_addr(magic6502_ctx* ctx) {
  return ctx->address_accessed = (*ctx->ma(ctx, ctx->pc + 1) | (*ctx->ma(ctx, ctx->pc + 2) << 8));
}
...
unsigned short relative_addr(magic6502_ctx* ctx) {
  return ctx->address_accessed = (ctx->pc + (char)*ctx->ma(ctx, ctx->pc + 1));
}
